...
const shared_ptr<Material> Material::byLabel(const std::wstring& label, Scene* w_){
	Scene* w=w_?w_:Omega::instance().getScene().get();
	FOREACH(const shared_ptr<Material>& m, w->materials){
		if(m->label == label) return m;
	}
	throw std::runtime_error((L"No material labeled `"+label+L"'.").c_str());
}

const int Material::byLabelIndex(const std::wstring& label, Scene* w_){
