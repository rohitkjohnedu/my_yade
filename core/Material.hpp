...
class Material: public Serializable, public Indexable{
	public:
		...
		*/
		...
		static const shared_ptr<Material> byLabel(const std::wstring& label, Scene* scene=NULL);
		static const shared_ptr<Material> byLabel(const std::wstring& label, shared_ptr<Scene> scene) {return byLabel(label,scene.get());}
		// return index of material, given its label
		static const int byLabelIndex(const std::wstring& label, Scene* scene=NULL);
	...
	YADE_CLASS_BASE_DOC_ATTRS_CTOR_PY(Material,Serializable,"Material properties of a :yref:`body<Body>`.",
		...
		((std::wstring,label,,,"Textual identifier for this material; can be used for shared materials lookup in :yref:`MaterialContainer`."))
