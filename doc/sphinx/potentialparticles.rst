...
The PP and PB codes are not currently included in the binary, pre-packaged version of ``yade``. They can be easily used though, either in ``yadedaily`` or if YADE is built and installed from source, following the :ref:`installation instructions<install-from-source-code>`.
