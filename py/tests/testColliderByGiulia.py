...
class TestColliderByGiulia(unittest.TestCase):
	def setUp(self):
		...
		sign=1
		ordering=True
		O.cell.setBox(Vector3(self.cellSizes[(0+axis)%3],self.cellSizes[(1+axis)%3],self.cellSizes[(2+axis)%3]))
		# Big blue sphere
...
#		from yade.wrapper import ForceEngine,NewtonIntegrator
		...
		#cellSizes[0]= self.DbigSphere*1.000000000000001 # ok - Anything bigger  than that works fine
		#cellSizes[0]= self.DbigSphere*0.605             # ok - Anything smaller than that works fine
