...
if 1:
	...
	assert(O.bodies[0].shape['edgeAdjIds'][1]==0 and O.bodies[1].shape['edgeAdjIds'][1]==1)
	assert(topo['commonEdgesFound']==1)
	assert(abs(O.bodies[0].shape['edgeAdjHalfAngle'][1]-(-.5*atan(old_div(2,sqrt(2)))))<1e-6)
...
if 1:
	...
	def sphPt(i,j):
		if i==0: return (0,0,-r)
		if i==old_div(nPoly,2): return (0,0,r)
		assert(i>0 and i<old_div(nPoly,2))
		assert(j>=0 and j<=nPoly)
		theta=old_div(i*2*pi,nPoly)
		rr=r*sin(theta)
		phi=old_div(j*2*pi,nPoly)
		return rr*cos(phi),rr*sin(phi),-r*cos(theta)
	for i in range(0,old_div(nPoly,2)):
		for j in range(0,nPoly):
			if i!=0: O.bodies.append(facet([sphPt(i,j),sphPt(i,j+1),sphPt(i+1,j)]))
			if i!=old_div(nPoly,2)-1: O.bodies.append(facet([sphPt(i+1,j),sphPt(i,j+1),sphPt(i+1,j+1)]))
	print('Sphere created, has',len(O.bodies),'facets')
	O.step()
	assert(topo['commonVerticesFound']==nPoly*(old_div(nPoly,2)-1)+2)
	assert(topo['commonEdgesFound']==nPoly*((old_div(nPoly,2)-1)+(old_div(nPoly,2)-2)*2+2))
