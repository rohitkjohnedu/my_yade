...
mn, mx= Vector3(5e-08, 5e-08, 5e-08), Vector3(maxval, 0.1, 0.1)
sp = pack.SpherePack();
...
O.engines=[
    ForceResetter(),
    InsertionSortCollider([Bo1_Sphere_Aabb(),Bo1_Box_Aabb(), Bo1_Subdomain_Aabb(), Bo1_FluidDomainBbox_Aabb()], label='collider'),
    InteractionLoop(
    ...
    GlobalStiffnessTimeStepper(timestepSafetyCoefficient=0.2, parallelMode=True, label = "ts"),
    fluidCoupling, #to be called after timestepper
    NewtonIntegrator(damping = 0.0, label='newton', gravity = (0, 0.0, 0)), 
    #VTKRecorderParallel(fileName='spheres/3d-vtk-', recorders=['spheres', 'intr', 'boxes'], iterPeriod=5000), 
    #PyRunner(command="printStep()", iterPeriod=1)
] 
